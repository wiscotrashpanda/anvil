...
variable "environments" {
  description = "Environment map. Each entry creates one TFE workspace, one StackSet, and one IAM role."
  type = map(object({
    aws = object({
      account_id             = string
...
variable "aws" {
  description = "AWS defaults for generated workspace environments."
  type = object({
