locals {
  ...
  github_repo_repository_inputs = {
    ...
    name => merge(
      ...
      try(manifest.spec.features.hasDiscussions, null) != null || try(manifest.spec.has_discussions, null) != null || try(manifest.spec.repository.features.hasDiscussions, null) != null || try(manifest.spec.repository.has_discussions, null) != null ? {
        has_discussions = coalesce(
          ...
          try(manifest.spec.has_discussions, null),
          try(manifest.spec.features.hasDiscussions, null),
        )
      } : {},
  ...
  github_repo_module_inputs = {
    for name, manifest in local.github_repo_manifests :
    name => {
      repository = local.github_repo_repository_inputs[name]

      create_terraform_workspaces = try(coalesce(
        try(manifest.spec.create_terraform_workspaces, null),
        try(manifest.spec.createTerraformWorkspaces, null),
      ), false)

      environments = try(manifest.spec.environments, null) == null ? {} : {
  ...
  managed_github_repo_module_inputs = local.github_repo_module_inputs

  terraform_workspace_repo_inputs = {
    for name, config in local.managed_github_repo_module_inputs :
    name => config
    if config.create_terraform_workspaces
  }
}
